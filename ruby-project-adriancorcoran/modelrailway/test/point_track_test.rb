...
class PointTrackTest < Minitest::Test
  # tried to load yaml data but couldn't - kept getting a load error

  def test_attrs
    my_track = PointTrack.new(Catalogue.new, 5, 10, 15, 3)
    assert_equal 5, my_track.cost
    assert_equal 10, my_track.quantity
    assert_equal 150, my_track.length
    assert_equal 3, my_track.num_points
  end

  def test_defaults
    my_track = PointTrack.new(Catalogue.new, 5, 10, 15)
    assert_equal 2, my_track.num_points
  end
